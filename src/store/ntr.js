...
export const useNtrStore = defineStore("ntr", () => {
  const flowData = reactive([...mergedData]);
  const thisWeekData = reactive([...sheetData]);

  // 평균 격노 계산 (숫자 변환 및 NaN 방지)
  const averageRage_ = computed(() => {
    const rageArr = thisWeekData.map((m) => Number(m["격노"])).filter((v) => !isNaN(v));
  ...
  const averageRage = computed(() => {
    ...
    const rageArr = thisWeekData
      .filter((m) => m["격노"] !== undefined && m["격노"] !== null && m["격노"] !== "")
      .map((m) => Number(m["격노"]))
      .filter((v) => !isNaN(v));
    if (rageArr.length === 0) return 0;
    return (rageArr.reduce((a, b) => a + b, 0) / rageArr.length).toFixed(2);
  ...
  const actualParticipants = computed(() => {
    return thisWeekData.filter((m) => !!m["격노"] == true).length;
  });

  // 격노별 개수 집계
  const rageCountData = computed(() => {
    const count = {};
    thisWeekData.forEach((m) => {
      const rage = m["격노"];
